#ifndef __BUBBLERICHEDITCTRL__H__
#define __BUBBLERICHEDITCTRL__H__
...
class BubbleRichEditCtrl:public ITextHost, public ITextEditControl
{
public:
	BubbleRichEditCtrl(void);
	~BubbleRichEditCtrl(void);

	BOOL Attach(HWND parent, RECT* rect, DWORD dwStyle);
...
private:
	...
	ITextServices* mITextServices;		// pointer to Text Services object
	IRichEditOle* mIRichEditOle;
	ITextDocument* mIDocument;
};
...
HRESULT CreateHost(HWND hwnd, const CREATESTRUCT *pcs, PNOTIFY_CALL pnotify, BubbleRichEditCtrl **pptwh);

#endif// __BUBBLERICHEDITCTRL__H__